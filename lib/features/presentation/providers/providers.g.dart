// GENERATED CODE - DO NOT MODIFY BY HAND

part of 'providers.dart';

// **************************************************************************
// RiverpodGenerator
// **************************************************************************

String _$dioHash() => r'6ce29c414410d27133e0cbe3fbe09d5ff949991b';

/// See also [dio].
@ProviderFor(dio)
final dioProvider = Provider<Dio>.internal(
  dio,
  name: r'dioProvider',
  debugGetCreateSourceHash:
      const bool.fromEnvironment('dart.vm.product') ? null : _$dioHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef DioRef = ProviderRef<Dio>;
String _$pokemonRemoteDataSourceHash() =>
    r'a7f5cf5e6cb7e506657cfa983756637f0f155570';

/// See also [pokemonRemoteDataSource].
@ProviderFor(pokemonRemoteDataSource)
final pokemonRemoteDataSourceProvider =
    Provider<PokemonRemoteDataSource>.internal(
  pokemonRemoteDataSource,
  name: r'pokemonRemoteDataSourceProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$pokemonRemoteDataSourceHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef PokemonRemoteDataSourceRef = ProviderRef<PokemonRemoteDataSource>;
String _$pokemonRepositoryHash() => r'9a9e0ec92dacaabe05ade8495ed4f5c2d5373460';

/// See also [pokemonRepository].
@ProviderFor(pokemonRepository)
final pokemonRepositoryProvider = Provider<PokemonRepository>.internal(
  pokemonRepository,
  name: r'pokemonRepositoryProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$pokemonRepositoryHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef PokemonRepositoryRef = ProviderRef<PokemonRepository>;
String _$getPokemonListUseCaseHash() =>
    r'af38cd063f88696873902652721dff7e0c1d9709';

/// See also [getPokemonListUseCase].
@ProviderFor(getPokemonListUseCase)
final getPokemonListUseCaseProvider = Provider<GetPokemonListUseCase>.internal(
  getPokemonListUseCase,
  name: r'getPokemonListUseCaseProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$getPokemonListUseCaseHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef GetPokemonListUseCaseRef = ProviderRef<GetPokemonListUseCase>;
String _$getPokemonDetailUseCaseHash() =>
    r'71519732134886499981bc4ef4e6c0665a3d7661';

/// See also [getPokemonDetailUseCase].
@ProviderFor(getPokemonDetailUseCase)
final getPokemonDetailUseCaseProvider =
    Provider<GetPokemonDetailUseCase>.internal(
  getPokemonDetailUseCase,
  name: r'getPokemonDetailUseCaseProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$getPokemonDetailUseCaseHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef GetPokemonDetailUseCaseRef = ProviderRef<GetPokemonDetailUseCase>;
String _$searchPokemonUseCaseHash() =>
    r'd2b23f61dcbafdfc451900949f4fcdd9a1362204';

/// See also [searchPokemonUseCase].
@ProviderFor(searchPokemonUseCase)
final searchPokemonUseCaseProvider = Provider<SearchPokemonUseCase>.internal(
  searchPokemonUseCase,
  name: r'searchPokemonUseCaseProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$searchPokemonUseCaseHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef SearchPokemonUseCaseRef = ProviderRef<SearchPokemonUseCase>;
// ignore_for_file: type=lint
// ignore_for_file: subtype_of_sealed_class, invalid_use_of_internal_member, invalid_use_of_visible_for_testing_member, deprecated_member_use_from_same_package
